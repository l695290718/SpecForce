...
import type { ArchitectureScopeRef, PrincipalAuthSource, ScopeAction, ScopedPrincipal, ScopeGrant } from "./types";
...
export function normalizePrincipalClaims(claims: PrincipalClaims, options: { allowSeed?: boolean } = {}): ScopedPrincipal {
  ...
  return { actorType: actorType as ScopedPrincipal["actorType"], actorId: subject, subject, tenantId, authSource, grants, permissions: normalizedPermissions, decisionRef };
}
