...
## Independent Review Queue Increment (2026-09-22)

The read-only MCP tool `read_knowledge_review_queue` now provides the missing human/Agent review boundary for finalized semantic scan candidates in the exact `com.specforge.designcenter` Scope. It requires a revalidated `DESIGN_CATALOG_CURATION` readiness receipt whose selector is bound to the requested Scan Session and whose purpose is `independent-semantic-review`; the caller must hold both `knowledge:read` and `governance:run`. A reviewer may read a session created by another actor in the same Scope, but review decisions remain actor-independent for T1-T3: the Scan Session actor and every candidate generator are rejected as reviewers. T0 remains the explicitly deterministic exception, while T2/T3 decisions require evidence at the server boundary.

The queue has two bounded projections. `BUNDLES` returns deterministic bundle summaries and global completeness; `CANDIDATES` returns only allowlisted bilingual fields, identity status, unresolved questions, evidence/source counts, and content digests. Raw candidate values, credentials, unrestricted source payloads, and arbitrary nested JSON are never exposed. BUNDLES default to 50/max 100 items and CANDIDATES default to 25/max 50; text is clamped, pages are capped at 256 KiB, and large reads fail closed with an explicit disclosure error.

PostgreSQL remains authoritative. Each first-page read creates an immutable `KnowledgeReviewQueueReceipt`; HMAC cursors carry key version, issue/expiry timestamps, exact Scope, reviewer, readiness receipt, queue receipt, projection, filters, page size, snapshot digest, and the deterministic order key. The review-set digest covers the finalized Scan Session, terminal semantic batch receipts, all ReviewBundle digests, and all existing decision digests. Continuation pages reject expired cursors or changed snapshots as stale. Graph stores receive no writes and remain derived projections.

Focused evidence for this increment: `pnpm db:generate` passed; `pnpm exec prisma migrate deploy` applied `20260922010000_add_review_queue_receipts` to `localhost:15433/specforge_canonical`; `pnpm --filter @specforge/mcp-server typecheck` passed; repository `pnpm typecheck` passed; `pnpm --dir apps/mcp-server exec vitest run src/knowledge/review-queue.test.ts src/knowledge/reviewer-policy.test.ts src/tools.test.ts src/knowledge/candidate-persistence.test.ts --exclude '../../.worktrees/**' --exclude '../../.pnpm-store/**'` passed 45 tests; and `SPECFORGE_KNOWLEDGE_INTEGRATION=1 pnpm exec vitest run apps/mcp-server/src/knowledge/persistence.integration.test.ts` passed the PostgreSQL persistence test. The queue implementation is complete for the governed read boundary; actual T1-T3 semantic approval, promotion, reconciliation, and production Baseline publication remain separate tracked work.

### 独立审核队列增量（2026-09-22）

只读 MCP 工具 `read_knowledge_review_queue` 已在精确 `com.specforge.designcenter` Scope 下补齐最终化语义扫描候选的人工/Agent 审核边界。调用必须携带重新核验的 `DESIGN_CATALOG_CURATION` 就绪回执，回执选择器绑定请求的 Scan Session，目的为 `independent-semantic-review`，且调用方同时拥有 `knowledge:read` 与 `governance:run`。同 Scope 内的授权审核人可以读取其他 Agent 创建的会话，但 T1-T3 的审核决策必须角色独立：扫描会话执行者以及候选生成者都会被拒绝为审核人。T0 仍是显式确定性事实的例外；T2/T3 在服务端强制要求证据。

队列提供两个有界投影。`BUNDLES` 返回确定性的审核包摘要和全局完整性；`CANDIDATES` 只返回白名单双语字段、身份状态、未决问题、证据/来源数量和内容摘要。不会暴露原始候选值、凭据、无限制来源载荷或任意嵌套 JSON。BUNDLES 默认 50 条/最多 100 条，CANDIDATES 默认 25 条/最多 50 条；文本会被截断，单页上限 256 KiB，超出时以明确的披露阻断错误失败关闭。

PostgreSQL 继续保持权威。首次读取会创建不可变 `KnowledgeReviewQueueReceipt`；HMAC 游标携带密钥版本、签发/过期时间、精确 Scope、审核人、就绪回执、队列回执、投影、过滤条件、页大小、快照摘要和确定性排序键。审核集合摘要覆盖最终化扫描会话、终端语义批次回执、全部 ReviewBundle 摘要和已有决策摘要。后续分页会拒绝过期游标或已变化快照；图数据库不接收写入，仍只作为派生投影。

本增量聚焦证据：`pnpm db:generate` 通过；`pnpm exec prisma migrate deploy` 已将 `20260922010000_add_review_queue_receipts` 应用到 `localhost:15433/specforge_canonical`；`pnpm --filter @specforge/mcp-server typecheck` 通过；仓库 `pnpm typecheck` 通过；`pnpm --dir apps/mcp-server exec vitest run src/knowledge/review-queue.test.ts src/knowledge/reviewer-policy.test.ts src/tools.test.ts src/knowledge/candidate-persistence.test.ts --exclude '../../.worktrees/**' --exclude '../../.pnpm-store/**'` 通过 45 项；`SPECFORGE_KNOWLEDGE_INTEGRATION=1 pnpm exec vitest run apps/mcp-server/src/knowledge/persistence.integration.test.ts` 通过 PostgreSQL 持久化测试。受治理的只读审核边界已完成；实际 T1-T3 语义批准、提升、对账和生产 Baseline 发布仍是独立待办。
