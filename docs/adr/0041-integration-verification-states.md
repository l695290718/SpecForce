...
- Backfill (design-time dry-run): `pnpm integration-attestations:backfill -- --application-service com.huawei.celon.desiner --scope-path pf-huawei/product-celon/subproduct-platform/module-celon-designer/com.huawei.celon.desiner --mode dry-run` → scanned 551, eligible 1, failed 0. **Correction (deployment): dry-run never writes; the canonical ledger held zero evidence→integration VALIDATES rows until the apply run below, so apply was required for reader cutover.**
...
- Deployment backfill (apply, canonical DB): `pnpm integration-attestations:backfill --mode apply --application-service com.huawei.celon.desiner --scope-path pf-huawei/product-celon/subproduct-platform/module-celon-designer/com.huawei.celon.desiner` → scanned 558, eligible 1, **upserted 1, failed 0, parity COMPLETE**.
- Parity (PostgreSQL, `specforge_canonical`): eligible `AssetLink` evidence→integration VALIDATES = 1, active canonical `RelationshipCurrent` VALIDATES evidence→integration = **1**; canonical row `legacy-asset-link:evidence:evidence-integration-specforge-mcp-agent-20260826:validates:integration:integration-specforge-mcp-agent`.
- Tests for the corrected read/write paths: `vitest run apps/web/lib/integrations` → **10 passed**; `vitest run apps/mcp-server/src/persistence.test.ts` → **492 passed** (incl. canonical evidence VALIDATES projection, backfill idempotency, backfilled-relationship update/invalidation in recorded enterprise).
- `pnpm exec tsc --noEmit -p apps/web` exit 0; `pnpm --filter @specforge/web build` compiled; deploy web image rebuilt with the `RelationshipCurrent` reader and container recreated (canonical database).
- Reader-cutover live verification (CDP, http://127.0.0.1:3010/assets/integrations?scope=com.huawei.celon.desiner): chips `Scopes inspected: 4 / Contracts scanned: 1 / Attested: 1 / Drift: 0`; contract row shows badge **Attested** with `Target resolution: UNRESOLVED`; drawer (edge click) shows `Verification state: ATTESTED` — now read from active canonical `RelationshipCurrent` rows, not `AssetLink`.
...
- `pnpm integration-attestations:backfill -- --application-service com.huawei.celon.desiner --scope-path pf-huawei/product-celon/subproduct-platform/module-celon-designer/com.huawei.celon.desiner --mode dry-run`：扫描 551 条，符合 1 条，失败 0 条。**修正（部署时）：dry-run 不写库，权威账本在下列 apply 之前没有任何 evidence→integration 的 VALIDATES 行，因此读取器切换必须先 apply。**
- 部署回填（apply，canonical 库）：`pnpm integration-attestations:backfill --mode apply ...` → 扫描 558 条，符合 1 条，**入账 1 条、失败 0 条、parity COMPLETE**。
- 对账（PostgreSQL `specforge_canonical`）：合格 `AssetLink` evidence→integration VALIDATES = 1，活动 `RelationshipCurrent` VALIDATES evidence→integration = **1**；规范行 `legacy-asset-link:evidence:evidence-integration-specforge-mcp-agent-20260826:validates:integration:integration-specforge-mcp-agent`。
- 修正后读写路径测试：`vitest run apps/web/lib/integrations` → **10 项通过**；`vitest run apps/mcp-server/src/persistence.test.ts` → **492 项通过**（含证据 VALIDATES 规范投影、回填幂等、回填关系在记录企业的更新/失效）。
- `pnpm exec tsc --noEmit -p apps/web` 退出码 0；`pnpm --filter @specforge/web build` 编译通过；部署 web 镜像已用 `RelationshipCurrent` 读取器重建并重建容器（canonical 库）。
- 读取器切换实机验证（CDP，http://127.0.0.1:3010/assets/integrations?scope=com.huawei.celon.desiner）：chips `Scopes inspected: 4 / Contracts scanned: 1 / Attested: 1 / Drift: 0`；契约行徽标 **Attested** 且 `Target resolution: UNRESOLVED`；抽屉（点击连线）显示 `Verification state: ATTESTED`——现在读取自活动 `RelationshipCurrent` 行而非 `AssetLink`。
