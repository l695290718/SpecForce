...
- Live Docker verification on 2026-08-26: existing PostgreSQL had no `_prisma_migrations` baseline, so `docker compose --env-file deploy/.env -f deploy/compose.yaml exec -T requirement-assessment-worker node_modules/.bin/prisma migrate deploy` returned Prisma `P3005`; the deployment topology intentionally uses Bootstrap `db push` for this adopted database. The already-built Worker image then ran `docker compose --env-file deploy/.env -f deploy/compose.yaml exec -T requirement-assessment-worker node_modules/.bin/prisma db push --schema=prisma/schema.prisma --skip-generate`, which completed successfully without reset or data-loss flags.
- Live Docker verification: PostgreSQL contained all eight Requirement Assessment tables (`RequirementBrief`, `RequirementAssessmentRun`, `AssessmentEvidenceSnapshot`, `RequirementAssessment`, `RequirementAssessmentTask`, `ModelProfile`, `AgentExecutionProfile`, `AssessmentExecutionActual`); `http://127.0.0.1:8093/healthz` returned `200` with `status=ok`, `lastTick.runs=[]`, and the exact Designer Scope; `http://127.0.0.1:3010/healthz` returned `200 {"status":"ok"}`; Bootstrap reported `COMPLETED` before the schema refresh and `SKIPPED` with the same inventory after it.
- Live deployment session: `design-change-session:ab458061-c741-4ddb-b891-85f01fc094ec`, preflight digest `8c9fbcb2af2808bbe89426d3ff4aab2ae852d3644d1d26c84571be68ee26aa05`.
...
- 2026-08-26 Docker 实时验证：存量 PostgreSQL 没有 `_prisma_migrations` 基线，运行 `docker compose --env-file deploy/.env -f deploy/compose.yaml exec -T requirement-assessment-worker node_modules/.bin/prisma migrate deploy` 返回 Prisma `P3005`；当前部署拓扑由 Bootstrap 使用 `db push` 管理这类已接管数据库。随后使用已构建 Worker 镜像执行 `docker compose --env-file deploy/.env -f deploy/compose.yaml exec -T requirement-assessment-worker node_modules/.bin/prisma db push --schema=prisma/schema.prisma --skip-generate` 成功，未使用 reset 或数据丢失参数。
- Docker 实时验证通过：PostgreSQL 已包含 8 张需求评估表（`RequirementBrief`、`RequirementAssessmentRun`、`AssessmentEvidenceSnapshot`、`RequirementAssessment`、`RequirementAssessmentTask`、`ModelProfile`、`AgentExecutionProfile`、`AssessmentExecutionActual`）；`http://127.0.0.1:8093/healthz` 返回 `200`、`status=ok`、`lastTick.runs=[]` 且为精确 Designer Scope；`http://127.0.0.1:3010/healthz` 返回 `200 {"status":"ok"}`；Bootstrap 在 Schema 刷新前返回 `COMPLETED`，刷新后以相同资产清单返回 `SKIPPED`。
- 实时部署会话：`design-change-session:ab458061-c741-4ddb-b891-85f01fc094ec`，预检摘要 `8c9fbcb2af2808bbe89426d3ff4aab2ae852d3644d1d26c84571be68ee26aa05`。
