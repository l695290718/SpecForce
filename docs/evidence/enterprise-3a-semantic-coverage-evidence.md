...
| `pnpm enterprise-3a:build` | After the final MCP design-fact synchronization, coverage generation `v10` published; Job `READY`; persisted error and diagnostic fields are empty. |
| `pnpm enterprise-3a:verify` | `CURRENT`; `297` rows; `297 COVERED`; `0 BLOCKED`; `0 NOT_EVALUATED`; maximum path `3`. |
...
- Manifest: `coverage-manifest:coverage-generation:designer:3a:v10`
- Catalog version: `303`
- Relationship version: `7664`
- Rows: `297`
- Covered: `297`
- Blocked: `0`
- Not evaluated: `0`
...
本次验证只针对精确 Designer Scope。目录水位线为 303，关系水位线为 7664，覆盖代次 v10 当前有效，297 条记录全部 COVERED，没有 BLOCKED 或 NOT_EVALUATED，路径上限为 3 跳。v6 结构基线仍为 8/42/6。所有 ADR、Proposal、Context Pack、Evidence 和类型关系通过 MCP 写入并按稳定 ID 读回。
