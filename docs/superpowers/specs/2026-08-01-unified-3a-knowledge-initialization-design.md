...
1. Generic foundation: configurable scopes and locales, versioned metamodel, assertions, identity candidates, ChangeSets, Streams, Baselines, and projection manifests.
...
交付依次分为通用基础、初始化治理、通用扫描、3A 投影、迁移与加固。持续同步、CMDB 和运行时连接器、出站 Proposal、外部 `APPLY`、自动跨 Scope 合并、生产对象存储和完整亿级基准测试保持延期。
