...
- Modify: `scripts/design-fact-manifest.test.ts` - keep the overview design-fact evidence contract aligned with the manifest.
...
- [ ] **Step 4: Update the manifest regression contract**

Run `scripts/design-fact-manifest.test.ts` after changing the overview evidence. Replace any stale fixed evidence-count assertion with assertions for the expected focused verification and MCP sync/check evidence entries so future evidence additions do not silently invalidate the governance contract.

- [ ] **Step 5: Commit evidence and synchronized repository record**

```bash
git add docs/adr/0013-architecture-overview-home.md docs/design-facts/baseline-manifest.json scripts/design-fact-manifest.test.ts
