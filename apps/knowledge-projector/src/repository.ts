...
const HEALTH_SQL = `SELECT COUNT(*) FILTER (WHERE status = 'QUEUED')::integer AS queued, COUNT(*) FILTER (WHERE status = 'BUILDING')::integer AS building, COUNT(DISTINCT "buildKey") FILTER (WHERE status = 'FAILED' AND NOT EXISTS (SELECT 1 FROM "ProjectionBuildJob" ready WHERE ready."applicationServiceId" = failed."applicationServiceId" AND ready."scopePath" = failed."scopePath" AND ready."profileId" = failed."profileId" AND ready."profileVersion" = failed."profileVersion" AND ready."projectionSchemaVersion" = failed."projectionSchemaVersion" AND ready.status = 'READY' AND ready."createdAt" > failed."createdAt"))::integer AS failed, EXTRACT(EPOCH FROM ($3::timestamptz - MIN("createdAt") FILTER (WHERE status = 'QUEUED')))::double precision AS oldest_queued_age_seconds, (SELECT MAX("publishedAt") FROM "ProjectionManifest" WHERE "applicationServiceId" = $1 AND "scopePath" = $2 AND "projectionSchemaVersion" = '3a.v2' AND "publishedAt" IS NOT NULL) AS last_published_at FROM "ProjectionBuildJob" failed WHERE "applicationServiceId" = $1 AND "scopePath" = $2`;
