...
describe("bounded 3A coverage report", () => {
  it("returns exact-Scope rows with a continuation cursor", async () => {
    ...
    const result = await get3aCoverageReport({ ...scope, limit: 2 }, fake as never, async () => undefined);
  ...
  it("rejects a mismatched Scope before reading data", async () => {
    await expect(get3aCoverageReport({ ...scope, scopePath: "other" })).rejects.toThrow();
  });
});
