...
export async function submitContinuousObservationBatchV2(input: SubmitContinuousObservationBatchV2Input) {
  ...
  return prisma.$transaction(async (transaction) => {
    ...
    const terminalDelta = input.batch.mode === "DELTA" && input.batch.isLastPage;
    await tx.connectorRun.update({ where: { applicationServiceId_scopePath_id: { ...scope, id: input.batch.runId } }, data: { acceptedSequence: input.batch.sequence, acceptedBatchDigest: input.batch.batchDigest, sourceCursor: input.batch.sourceCursor, sourceHighWaterMark: input.batch.sourceHighWaterMark, status: terminalDelta ? "SUCCEEDED" : input.batch.isLastPage ? "FINALIZING" : "RUNNING", finishedAt: terminalDelta ? acceptedAt : undefined, coverage: json(input.batch.coverage) } });
    if (terminalDelta) await tx.connectorLease.deleteMany({ where: { ...scope, connectorId: input.batch.connectorId, sourceNamespace: input.batch.sourceNamespace, runId: input.batch.runId } });
