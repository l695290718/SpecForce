...
export default async function FeaturesPage({ searchParams }: { searchParams: Promise<{ scope?: string; locale?: string; view?: string; q?: string; cursor?: string; selection?: string }> }) {
  ...
  const [selected, graph] = await Promise.all([params.selection && view !== "graph" ? getScopedFeatureDetail(scope, params.selection, locale, principal) : undefined, view === "graph" ? getScopedFeatureGraph(scope, { depth: 2, limit: 300, locale }, principal) : undefined]);
