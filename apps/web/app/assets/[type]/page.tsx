...
export default async function AssetListPage({ params, searchParams }: { params: Promise<{ type: AssetRouteType }>; searchParams: Promise<{ q?: string }> }) {
  ...
  return (
    <>
      <PageHeader title={<T k={assetTitleKeys[type]} />} description={<T k="asset.browseDescription" />} action={<ButtonLink href={`/assets/${type}/new`}><T k="action.new" /></ButtonLink>} />
      <Card className="mb-4">
      ...
      <DataTable
        ...
        rows={assets.map((asset) => [
          <div key="name">
            <Link className="font-semibold text-ink hover:text-accent" href={`/assets/${type}/${asset.id}`}>{assetName(asset)}</Link>
          ...
          <Link className="font-medium text-accent" href={`/assets/${type}/${asset.id}`} key="details"><T k="action.details" /></Link>
        ])}
      />
    </>
  );
