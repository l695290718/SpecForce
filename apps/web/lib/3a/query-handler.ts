...
export async function handleThreeAQuery(request: Request, dependencies: ThreeAQueryHandlerDependencies): Promise<Response> {
  try {
    ...
    if (parsed.operation === "unitGraph") {
      return Response.json(await dependencies.createService().unitGraph({ ...identity, generationId: parsed.generationId, includeMembers: parsed.includeMembers, includeMemberRelations: parsed.includeMemberRelations, filter: parsed.filter ?? {}, budget: parsed.budget, continuation: parsed.continuation }));
    }
