...
const ArchitectureGraphExplorer = dynamic(() => import("./architecture-graph-explorer").then((module) => module.ArchitectureGraphExplorer), { ssr: false });
...
export function ThreeAWorkspace({ data }: { data: ThreeAWorkspaceData }) {
  ...
  return <div className="space-y-4 pb-8" data-testid="three-a-workspace">
    ...
    {data.state.tab === "alignment" ? <AlignmentView edges={alignmentEdges} /> : data.state.tab === "drift" ? <PublishedBaselineDriftView drift={data.drift} /> : data.state.mode === "graph" ? (data.initialGraph && catalogIdentity ? <ArchitectureGraphExplorer initialGraph={data.initialGraph} identity={catalogIdentity} onFocus={setFocusId} /> : <section className="rounded-lg border border-border bg-white p-8 text-center shadow-panel"><p className="text-sm font-semibold text-ink"><T k="threeA.selectGraphFocus" /></p><a className="mt-3 inline-flex text-sm font-semibold text-accent underline" href={tabHref({ ...data.state, mode: "lanes" }, "architecture")}><T k="threeA.lanes" /></a></section>) : <>
