...
describe("SigmaArchitectureGraph adapters", () => {
  ...
  it("limits Sigma reducers and layout requests to an explicitly visible subgraph", () => {
    const store = createArchitectureGraphStore(identity);
    const node = { stableId: "fact:one", kind: "fact" as const, label: "one", layer: "SYS" as const, memberCount: 1, degree: 1, criticality: 0, x: 0, y: 0 };
    store.graph.addNode("fact:one", node);
    store.graph.addNode("fact:two", { ...node, stableId: "fact:two" });
    store.graph.addNode("fact:three", { ...node, stableId: "fact:three" });
    store.graph.addDirectedEdgeWithKey("relationship:visible", "fact:one", "fact:two", { stableId: "relationship:visible", relationCode: "CALLS", confidence: 1, bridge: false, weight: 1 });
    store.graph.addDirectedEdgeWithKey("relationship:hidden", "fact:two", "fact:three", { stableId: "relationship:hidden", relationCode: "CALLS", confidence: 1, bridge: false, weight: 1 });
    const visibleNodeIds = new Set(["fact:one", "fact:two"]);
    const visibleEdgeIds = new Set(["relationship:visible"]);
    const request = createLayoutRequest(store, "overview", "force", false, visibleNodeIds, visibleEdgeIds);
    const settings = createSigmaSettings(store, { current: undefined }, { current: undefined }, { current: undefined }, { current: false }, { current: undefined }, { current: undefined }, undefined, { current: new Set() }, { current: new Set() }, { current: visibleNodeIds }, { current: visibleEdgeIds });

    expect(request.nodes.map(({ id }) => id)).toEqual(["fact:one", "fact:two"]);
    expect(request.edges).toEqual([{ source: "fact:one", target: "fact:two", weight: 1 }]);
    expect(settings.nodeReducer("fact:three", store.graph.getNodeAttributes("fact:three"))).toMatchObject({ hidden: true, size: 0 });
    expect(settings.edgeReducer("relationship:hidden", store.graph.getEdgeAttributes("relationship:hidden"))).toMatchObject({ hidden: true, size: 0 });
    expect(settings.edgeReducer("relationship:visible", store.graph.getEdgeAttributes("relationship:visible")).hidden).toBe(false);
  });
