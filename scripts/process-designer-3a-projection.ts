...
async function main(): Promise<void> {
  ...
  const materializer = new ProjectionMaterializer(repository, {
    owner: "designer-3a-verification",
  ...
  const owner = "designer-3a-verification";
  const latest = await prisma.projectionBuildJob.findFirst({ where: { ...scope, baselineId }, orderBy: { createdAt: "desc" } });
  if (!latest) throw new Error("DESIGNER_3A_PROJECTION_JOB_NOT_FOUND");
  const leaseExpiresAt = new Date(Date.now() + 30_000);
  await prisma.projectionBuildJob.update({ where: { applicationServiceId_scopePath_id: { ...scope, id: latest.id } }, data: { status: "BUILDING", leaseOwner: owner, leaseExpiresAt, errorCode: null, diagnosticRef: null } });
  const job: ProjectionBuildJob = {
    ...scope,
    id: latest.id,
    buildKey: latest.buildKey,
    ...
    checkpoint: (latest.checkpoint ?? {}) as ProjectionBuildJob["checkpoint"],
...
void main().catch(async (error) => { console.error(error); process.exitCode = 1; });
