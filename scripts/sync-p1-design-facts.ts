...
async function main() {
  ...
  const fixtureAdr = adr(fixtureAdrId, "Isolate graph verification fixtures", "Keep verification fixtures in a dedicated verification Scope and remove them only through MCP.", "Product Scope data must never be polluted by graph verification records.", ["Reuse the product Scope", "Delete fixtures directly from a graph store"], ["Verification is repeatable and product reads remain isolated; live Gateway readiness is still external."], ["Exact verification Scope", "MCP-only deletion", "PostgreSQL authority"], { name: "隔离图验证夹具", description: "将验证夹具放入独立 Scope，并只通过 MCP 清理。", title: "隔离图验证夹具", context: "产品 Scope 不能被图验证记录污染。", decision: "使用独立验证 Scope，并只通过 MCP 删除。", alternatives: ["复用产品 Scope", "直接从图数据库删除夹具"], consequences: ["验证可重复且产品读取保持隔离；实时网关就绪仍是外部条件。"], constraints: ["精确验证 Scope", "只能通过 MCP 删除", "PostgreSQL 权威"] });
  const fixtureProposal = proposal(fixtureProposalId, "Implement graph verification fixture isolation", "The live graph checker previously used product data and could leave verification records behind.", "Make fixture setup bounded by run ID, isolated by Scope, and recoverable through MCP cleanup.", "The verification Scope and historical fixture cleanup path.", ["Register verification-only Scope", "Add fingerprint-protected cleanup", "Add fail-closed live configuration"], ["Nebula Gateway may be unavailable", "Historical IDs must not be deleted without fingerprint validation"], { name: "实施图验证夹具隔离", background: "实时图检查器此前可能使用产品数据并遗留验证记录。", goal: "让夹具按运行 ID 限定、按 Scope 隔离，并可通过 MCP 恢复清理。", nonGoal: "不包含实时 Nebula 部署。", scope: "验证 Scope 和历史夹具清理路径。", specChanges: ["注册验证专用 Scope", "增加指纹保护清理", "增加失败关闭的实时配置"], risks: ["Nebula Gateway 可能不可用", "没有指纹校验不得删除历史 ID"], rolloutPlan: "先清理同一运行批次，再执行验证，最后通过 finally 清理。", rollbackPlan: "使用恢复清理阶段删除显式运行批次。" });
  const fixtureContext = contextPack(fixtureContextId, fixtureProposalId, "Graph Verification Fixture Context Pack", "Exact Scope and cleanup rules for graph verification fixtures.", ["Never use product Scope data", "Delete only explicit run IDs", "Record Gateway blocking as deferred"], ["Use the verification Scope", "Validate historical fingerprints", "Run cleanup after every verification"], "# Graph verification fixtures\n\nUse the exact verification Scope and MCP cleanup.\n", { name: "图验证夹具 Context Pack", summary: "图验证夹具的精确 Scope 与清理规则。", constraints: ["绝不使用产品 Scope 数据", "只删除显式运行 ID", "把 Gateway 阻塞记录为延期"], instructions: ["使用验证 Scope", "校验历史指纹", "每次验证后执行清理"], generatedMarkdown: "# 图验证夹具\n\n使用精确验证 Scope 和 MCP 清理。\n" });
  const fixtureEvidence = evidence(fixtureEvidenceId, fixtureAdrId, "Graph verification fixture cleanup evidence", "Scope isolation, fingerprint cleanup, and the externally blocked live Gateway check.", "pnpm exec tsx scripts/cleanup-graph-verification-fixtures.ts --phase delete; pnpm exec tsx deploy/graph/live-projection-check.ts --phase prepare; pnpm exec tsx deploy/graph/live-projection-check.ts --phase cleanup", "Historical 9 records deleted through MCP; live prepare blocked by NEBULA_LIVE_GATEWAY_UNAVAILABLE; recovery cleanup deleted 3 assets with remainingLinks=0.", "blocked", { name: "图验证夹具清理证据", description: "Scope 隔离、指纹清理和外部网关阻塞的实时检查证据。", command: "pnpm exec tsx scripts/cleanup-graph-verification-fixtures.ts --phase delete；pnpm exec tsx deploy/graph/live-projection-check.ts --phase prepare；pnpm exec tsx deploy/graph/live-projection-check.ts --phase cleanup", result: "通过 MCP 删除历史 9 条记录；实时 prepare 被 NEBULA_LIVE_GATEWAY_UNAVAILABLE 阻塞；恢复清理删除 3 个资产且 remainingLinks=0。" });
  const fixtureLinks = [
