...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (!manifest) throw new Error("DESIGNER_3A_V4_MANIFEST_NOT_FOUND");
    ...
    if (!scopeMatches || units.length !== 4 || mappings.length !== 3 || map.status !== "READY") throw new Error(`DESIGNER_3A_V4_READBACK_MISMATCH:${JSON.stringify({ scopeMatches, status: map.status, units: units.length, mappings: mappings.length })}`);
