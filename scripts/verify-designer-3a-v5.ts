...
async function main(): Promise<void> {
  ...
  try {
    ...
    const members = await prisma.architectureUnitMemberProjection.findMany({ where: { ...scope, generationId: String(identity.generationId), baselineId, projectionManifestId }, select: { unitIdentity: true, assertionId: true, semanticIdentity: true } });
    const memberAssetIds = members.map((member) => String(member.assertionId ?? member.semanticIdentity ?? ""));
    const gatewayMembers = members.filter((member) => member.unitIdentity === gatewayUnit);
    ...
    if (members.length !== 38 || memberCount !== 38 || gatewayMembers.length !== 23 || missingExpected.length || Number(map.unclassifiedCount ?? -1) !== 0) throw new Error(`V5_MEMBER_READBACK_MISMATCH:${JSON.stringify({ members: members.length, memberCount, gatewayMembers: gatewayMembers.length, missingExpected, unclassifiedCount: map.unclassifiedCount })}`);
    ...
    console.log(JSON.stringify({ scope, baselineId, projectionManifestId, generationId: identity.generationId, status: map.status, units: units.length, members: members.length, gatewayMembers: gatewayMembers.length, memberCount, mappings: mappings.length, totalByLayer: map.totalByLayer, unclassifiedCount: map.unclassifiedCount, fixtureHits, deferredHits, missingExpected, scopeMatches, memberReadback: "postgresql-derived-projection-read-only" }, null, 2));
  } finally {
