...
const webAuthMode = (process.env.SPECFORGE_WEB_AUTH_MODE ?? (mode === "local-account" ? "local-account" : "static")).toLowerCase();
...
if (mode === "local-account") {
  ...
  if (webAuthMode !== "local-account") failures.push("SPECFORGE_WEB_AUTH_MODE must be local-account when identity mode is local-account");
  if (pepper.toLowerCase().includes("replace-with")) failures.push("SPECFORGE_IDENTITY_SECRET_PEPPER still contains the placeholder value");
...
else console.log(JSON.stringify({ status: "VALID", mode, webAuthMode, database: "postgresql", staticBearer: mode === "development" }, null, 2));
