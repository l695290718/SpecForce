...
async function main(): Promise<void> {
  ...
  const client = new Client({ name: "designer-3a-v5-projection", version: "0.1.0" }, { capabilities: {} });
  ...
  try {
    ...
    const materializer = new ProjectionMaterializer(repository, {
      owner: "designer-3a-v5-projection",
      ...
      architectureUnitSource: async (job) => {
        ...
        if (units.length !== 4 || memberships.length !== 38 || mappings.length !== 3) throw new Error(`V5_PROJECTION_SOURCE_COUNT_MISMATCH:${JSON.stringify({ units: units.length, memberships: memberships.length, mappings: mappings.length })}`);
